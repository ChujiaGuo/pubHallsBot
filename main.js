...
client.on("message", async message => {
    ...
    if (message.channel.type == "dm" && !activeDMs.includes(message.author.id)) {
        ...
        if (processes.pendingRestart && cmd != "restart") return message.channel.send("Bot is pending a restart. Please try again later.")
...
client.on("guildMemberUpdate", async (oldMember, newMember) => {
    ...
    otherServerConfig = JSON.parse(fs.readFileSync(`./configs/${otherServer.id}.json`))
    ...
    var thisServerConfig = JSON.parse(fs.readFileSync(`./configs/${newMember.guild.id}.json`))
    var otherServerConfig = JSON.parse(fs.readFileSync(`./configs/${otherServer.id}.json`))
