...
exports.run = async (client, message, args, Discord, sudo = false) => {
    ...
    for (var i in allMembers) {
        ...
        let memberNames = getNames(member)
    ...
    for (var n in duplicatenames) {
        let members = allMembers.find(m => m.nickname.replace(/[a-z|]/gi, "").toLowerCase().split("|").includes(duplicatenames[n].toLowerCase()))
        duplicateMembers.concat(members)
    }
